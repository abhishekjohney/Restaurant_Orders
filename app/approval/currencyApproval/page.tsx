...
const CurrencyApproval = () => {
  ...
  const handleDelete = (item: CurrencyInterface) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await listAPI.getDeleteNewExpense(item?.NCRID);
          if (response) {
  ...
  const formatDatingDate = (date: any) => {
  ...
  const handleApprove = async (item: CurrencyInterface, status: boolean | string) => {
    let title;
    let aprSts: string | boolean;
    if (status === true) {
      aprSts = true;
    ...
    Swal.fire({
      ...
      didOpen: () => {
        ...
        // dateInput.value = formatDate(updatedDate);
      },
    }).then(async (result) => {
      if (result.isConfirmed) {
        // const dateInputValue = document.getElementById("dateInput").value;
        // const dateInputValue = document.getElementById("dateInput") as HTMLInputElement;
        // dateInputValue.value = formatDate(updatedDate);
        try {
          setLoading(true);
          //   const ReAprUser = session?.data?.user?.name;
          const fetchDetails = await updateAPI.updateCurrencyApproval(userName, item.NCRID, aprSts);
          if (fetchDetails) {
                ...
                >
                  <div className="p-4">
                    ...
                    <div className="flex justify-between">
                      <div className="flex justify-between">
                        {item?.AprSts !== 0 ? (
                          <>
                            ...
                            >
                              <Image height={500} width={500} className="size-10 block" src={approve} alt="payment update" />
                            </button>
                          </>
                        ) : (
                          <>
                            <button
                              ...
                              className={`block w-fit px-3 bg-white rounded shadow-lg  p-1`}
                            >
                              <Image height={500} width={500} className="size-10 block" src={reject} alt="payment update" />
                            </button>
                          </>
                        )}
                      </div>
                      <div>
                        <button
                          onClick={() => handleApprove(item, "delete")}
                          disabled={item?.AprSts === 1}
                          className={`block w-fit bg-[#ff1d1d] disabled:bg-[#ffadad] text-white px-3 rounded-lg p-1`}
                        >
                          Delete
                        </button>
                      </div>
            ...
            <table className=" divide-gray-200 overflow-hidden table ">
              <thead className="bg-primary sticky top-0">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-warning-content uppercase tracking-wider">Approved Date</th>
                </tr>
              </thead>
                ...
                {filteredExpenseList &&
                  filteredExpenseList?.map((item, ind) => (
                    <tr key={ind}>
                      ...
                      <td onClick={() => handleEdit(item)} className="px-6 py-4 whitespace-nowrap cursor-pointer">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex gap-2">
                          {item?.AprSts !== 0 ? (
                            <>
                              <button onClick={() => handleApprove(item, false)} className={`block w-fit bg-[#88efff] px-3 rounded-lg p-1`}>
                                <Image height={500} width={500} className="size-10 block" src={approve} alt="payment update" />
                              </button>
                            </>
                          ) : (
                            <>
                              <button onClick={() => handleApprove(item, true)} className={`block w-fit bg-[#88efff] px-3 rounded-lg p-1`}>
                                <Image height={500} width={500} className="size-10 block" src={reject} alt="payment update" />
                              </button>
                            </>
                          )}
