...
const TableSeatSelector: React.FC<Props> = ({
  ...
  // ✅ Step 2: Group occupied seats by OrderID (for this counter only)
  const occupiedGroups = counterData
    .filter(
  ...
  // ✅ Step 3: Get only tables for the selected counter
  const tables = counterData.filter((item) => item.SeatType === "Table");
        ...
        {isOccupiedOpen && (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {Object.entries(occupiedGroups).map(([orderId, seats]) => {
              ...
              let orderNo;
              if (entryDateStr) {
                orderNo = data.find((d) => d.OrderID.toString() === orderId)?.OrderNo || "";
              }
              let NetAmount;
              if (entryDateStr) {
                NetAmount = data.find((d) => d.OrderID.toString() === orderId)?.Amount || "";
              }
