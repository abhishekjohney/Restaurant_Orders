...
const dailyExpenses = [
    ...
    {
        type: "Entertainment",
        description: "Tickets for client event",
    },
    {
        type: "Supplies",
    ...
    {
        type: "Communication",
        description: "Phone bill for business calls",
    },
];
...
const TodayExpense = () => {
    ...
    const [partyList, setPartyList] = useState(dailyExpenses);
    ...
    useEffect(() => {
        const filteredList = partyList.filter((item) => item?.type.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    const handleOrder = async (selectedItem: PartyItemType) => {
        ...
        setSearchQuery(selectedItem?.type);
        setFormdData({
            ...formData,
            accountHead: selectedItem.type,
        });
        setShowModal(false);
    };
