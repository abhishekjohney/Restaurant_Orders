...
import locationUpdate from "../../../public/images/svg/locationUpdate.svg";
...
const PartyList = () => {
  ...
  const handleLocation = async (item: PartyItemType) => {
    if (item?.LocAprYN) return toast.success("Location already updated");
    setLoading(true);
    const fetchDetails = await listAPI.getPartyLocationDetails(item?.AccAutoIDClient);
    const jsonData1Array = JSON.parse(fetchDetails[0].JSONData1);
    if (jsonData1Array[0]) {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          async (position) => {
            ...
            const response = await listAPI.getUserLocationDetails(latitude.toString(), longitude.toString());
            setLocationData({
              Address: response?.display_name,
              latitude: latitude.toString(),
              longitude: longitude.toString(),
              existing: false,
              PartyAddress: item?.AccAddress,
              PartyId: item?.AccAutoIDClient,
              LocAprYN: item?.LocAprYN,
              PartyName: item?.Byr_nam,
              place: getLocationPlace(response),
              edit: true,
              existingAddress: "",
              existingLatLong: "",
              existingPlace: "",
          ...
          (error) => {
            ...
            setLocationData({
              ...
              PartyAddress: item?.AccAddress,
              PartyId: item?.AccAutoIDClient,
              existingAddress: item?.LocationString,
              existingLatLong: item?.LocLatLong,
              existing: true,
              PartyName: item?.Byr_nam,
              LocAprYN: item?.LocAprYN,
              place: "Not Allowded",
                ...
                {partyFilterList &&
                  partyFilterList.map((item: PartyItemType) => (
                    <tr className=" cursor-pointer" key={item.AccAutoID}>
                      ...
                      <td className="px-2 py-4 w-fit">{item.PartyRemarks}</td>
                    </tr>
                  ))}
              </tbody>
            </table>
